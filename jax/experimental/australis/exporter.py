...
def arguments(staged: Lowered) -> str:
  ...
  for i in range(arity):
    ...
    if i in donated_args:
      arg_text.append(f'aux::PTree x{i}')
    else:
      arg_text.append(f'const aux::PTree& x{i}')
...
def embed_c_string(bin_text, *, sym, file):
  ...
  print(
      """static constexpr absl::string_view %s = {
%s, %d};
...
absl::StatusOr<PTree>
{ name }::operator()({ args }) {{
  // A vector (per-shard) of vectors (flattened arguments) of PjRtBuffers (arg).
...
class { name } : public aux::internal::AustralisComputation {{
 public:
  // Compiles the Australis-staged program and returns a callable functor.
  static absl::StatusOr<{ name }> Load(aux::Client client);

  // Executes the function on the supplied data.
  absl::StatusOr<aux::PTree>
  operator()({ args });

 private:
  {name}(
      aux::internal::AustralisComputation computation)
