...
def serialize_executable_spec(fn):
  ...
  if is_pjit(fn):
    num_replicas = 1
    _, fs = pxla._get_backend_from_shardings(
        fn._lowering.compile_args['in_shardings'] + fn._lowering.compile_args['out_shardings'])  # pytype: disable=attribute-error
    num_partitions = len(fs._device_assignment())
    use_spmd = 1
  else:
    num_replicas = 1
    num_partitions = 1
