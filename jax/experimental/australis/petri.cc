...
class PTreeStorage {
 public:
  ...
  virtual absl::StatusOr<std::unique_ptr<xla::Literal>> ToLiteral() const = 0;
  virtual void ToLiteralAsync(
      std::function<void(absl::StatusOr<std::shared_ptr<xla::Literal>>)>
          on_ready) const = 0;
  virtual absl::StatusOr<std::vector<std::unique_ptr<xla::Literal>>>
  ToLiteralShards() const = 0;
...
class PTreeTuple : public PTreeStorage {
 public:
  ...
  absl::StatusOr<std::unique_ptr<xla::Literal>> ToLiteral() const override {
    return absl::InvalidArgumentError("Cannot get a literal out of a tuple!");
  }
  void ToLiteralAsync(
      std::function<void(absl::StatusOr<std::shared_ptr<xla::Literal>>)>
          on_ready) const override {
    on_ready(
        absl::InvalidArgumentError("Cannot get a literal out of a tuple!"));
  }
  absl::StatusOr<std::vector<std::unique_ptr<xla::Literal>>> ToLiteralShards()
      const override {
    return absl::InvalidArgumentError(
...
class PTreeDeviceArray : public PTreeStorage {
 public:
  ...
  void ToLiteralAsync(
    ...
    data_.ToLiteralAsync(0, on_ready);
  }
  ...
  absl::StatusOr<std::vector<std::unique_ptr<xla::Literal>>> ToLiteralShards()
    ...
    auto result = data_.ToLiterals();
    if (!result.ok()) return result.status();
    return std::vector<std::unique_ptr<xla::Literal>>(
...
absl::StatusOr<std::unique_ptr<xla::Literal>> PTree::ToLiteral() const {
  return ptr_->ToLiteral();
}

void PTree::ToLiteralAsync(
    std::function<void(absl::StatusOr<std::shared_ptr<xla::Literal>>)> on_ready)
    const {
  return ptr_->ToLiteralAsync(std::move(on_ready));
}

absl::StatusOr<std::vector<std::unique_ptr<xla::Literal>>>
PTree::ToLiteralShards() const {
  return ptr_->ToLiteralShards();
}
