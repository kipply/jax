...
absl::string_view PrimitiveTypeToString(PrimitiveType type) {
  switch (type) {
...
std::string Array::ToString() const {
  ...
  absl::StrAppend(&out, " ");
  AppendRecursive(&out, aval_.dtype, data(), 0, byte_strides(), aval().shape);
  return out;
}

Array Array::operator[](size_t idx) const {
  auto new_shape = absl::Span<int64_t const>(aval_.shape).subspan(1);
  return Array({aval_.dtype, Shape(new_shape.begin(), new_shape.end())},
      reinterpret_cast<const char*>(
          data()) + idx * byte_strides_[0],
      byte_strides().subspan(1),
      owner_);
}
